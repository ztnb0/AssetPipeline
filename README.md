...
统一搜索结果按“图片/视频”以及“内部资源/外部资源”分组。内部图片和视频分别按内部综合得分返回前 20；外部图片按固定平台优先级拼接，每个平台保留平台原始排名并最多返回 20 条，优先级为包图网、Pexels、Pixabay、Unsplash、Openverse、Mixkit；不对外部结果再次执行 Embedding。外部视频保留各平台原始顺序，每个平台返回前 10。交互式检索不会调用生成式模型扩词，查询向量会在后端进程内缓存，图片和视频检索可复用同一个向量。
